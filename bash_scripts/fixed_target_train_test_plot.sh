...
if i_am_on_slurm; then
    #I'm on slurm
    train_job_id=$(sbatch --cpus-per-task=1 --output=$(pwd)/outputs/${output_directory}/train/train.out bash_scripts/train.sh '--no-change_target' ${output_directory} | awk '{print $4}')
    test_job_id=$(sbatch --cpus-per-task=1 --dependency=afterok:$train_job_id --output=$(pwd)/outputs/${output_directory}/test/test.out bash_scripts/test.sh '--no-change_target' ${output_directory} | awk '{print $4}')
