...
if i_am_on_slurm; then
    ...
    plot_job_id=$(sbatch --dependency=afterok:$train_job_id --output=$(pwd)/outputs/${output_directory}/test/plot.out bash_scripts/plot.sh '--double' '--plot_test' ${output_directory}| awk '{print $4}')
