...
initial_action={'m1':np.degrees(initial_pose[0]).item(),'m2':np.degrees(initial_pose[1]).item(),'m3':np.degrees(initial_pose[2]).item(),
                'm2':np.degrees(initial_pose[3]).item(),'m5':np.degrees(initial_pose[4]).item(),'m6':np.degrees(initial_pose[5]).item()}
...
def convert_actions(new_actions):
    ...
    motor_positions_deg = {
        'm1': new_actions[0].item(),
        'm2': new_actions[1].item(),
        'm3': new_actions[2].item(),
        'm4': new_actions[3].item(),
        'm5': new_actions[4].item(),
        'm6': new_actions[5].item()
    }
...
def process_motor_positions(motor_positions):
    ...
    for i in range(6):
        obs_list.append(motor_positions[f'm{i+1}_position'])  # Assume each motor position is a list
    
    # Flatten motor speeds
    for i in range(6):
        obs_list.append(motor_positions[f'm{i+1}_speed'])  # Assuming speed is a single value (not a list)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:  # Loop to receive positions and send actions multiple times
            ...
            time.sleep(1)  # Optional: wait before sending the next set of actions
    finally:
